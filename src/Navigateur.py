...
class Navigateur:
    ...
    def __init__(self, proxy = None, proxySock = False):
        ...
        self.userAgent = random.choice(self.listeUserAgents)
    ...
    def getFichier(self, url, referer = None):
        try:
            ...
            # si le serveur renvoie un flux gzippé, il faut le dézipper...
            if page.info().get('Content-Encoding') == 'gzip':
                tampon = StringIO.StringIO(page.read())
                donnees = gzip.GzipFile(fileobj=tampon).read()
            else:
                donnees = page.read()
            return donnees
